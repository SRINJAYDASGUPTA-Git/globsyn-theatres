...
export class AdminComponent implements OnInit {
  ...
  loadMovies() {
    this.movieService.movieGet().subscribe(
      (data) => {
        this.movies = data;
      },
      (error) => {
  ...
  loadScreens() {
    this.screenService.screenGet().subscribe(
      (data) => {
        this.screens = data;
      },
      (error) => {
  ...
  onSubmit(): void {
    if (this.movieScheduleForm.valid) {
      ...
      this.movieService.moviePost(formValues.movie).subscribe(
        (movieResponse) => {
          const schedule = {
            ...formValues.schedule,
            movie: movieResponse._id,
          };
          this.scheduleService.schedulePost(schedule).subscribe(
            (scheduleResponse) => {
